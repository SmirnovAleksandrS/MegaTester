...
void readDiode(struct Diode * diode){
	 ...
	 *               |       |   |
	 *              V1       V2  R2
	 * V1 - точка первого замера
	 * V2 - точка второго замера
	 * Вычисляем ток протекающий через диод как V2/R2
	 * Вычисляем напряжение открытия диода V0 = V1-V2
	 *
	 * Если при токе > 25nA напряжение открытия больше 0, т.е адекватное,
	 * то мы нашли открытое положение диода
	 *
	 * Иначе проверяем обратное включение
	 *
	 * Второй замер:
	 *
	 * 0V----|==|-----<|------|==|-----3.3V
	 ...
	 * V1 - точка первого замера
	 * V2 - точка второго замера
	 * Вычисляем ток протекающий через диод как V1/R1
	 * Вычисляем напряжение открытия диода V0 = V2-V1
	 *
	 * Если при токе > 25nA напряжение открытия больше 0, т.е адекватное,
	 * то мы нашли открытое положение диода
	 *
	 * Иначе все тлен и диод хрень.
	 */
